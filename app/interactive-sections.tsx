...
export function ProductDemo() {
  ...
  function selectReply(question:string) { setReply(aiReplies[question]); setHandoff(question === "Talk to a staff member"); }
  ...
  return <section className="demo-section" id="demo"><div className="section-title split-title"><div><span className="eyebrow">PRODUCT DEMONSTRATION</span><h2>See how the systems work together.</h2></div><p>Explore a simplified demonstration of how Northstar handles bookings, sales, inventory, and customer questions.</p></div><p className="demo-disclaimer">Sample data shown for demonstration purposes.</p>
    <div className="demo-shell"><div className="demo-tabs" role="tablist" aria-label="Product demonstrations">{tabs.map(tab => <button key={tab.id} id={`demo-tab-${tab.id}`} role="tab" aria-controls={`demo-panel-${tab.id}`} aria-selected={demo === tab.id} tabIndex={demo === tab.id ? 0 : -1} onClick={() => setDemo(tab.id)}>{tab.label}</button>)}</div>
      <AnimatePresence mode="wait"><motion.div id={`demo-panel-${demo}`} role="tabpanel" aria-labelledby={`demo-tab-${demo}`} className="demo-canvas" key={demo} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:.24}}>
        {demo === "booking" && <div className="booking-demo"><div className="demo-summary"><span>SIMULATED BOOKING FLOW</span><h3>{booked ? "Booking confirmed." : "Choose an available appointment."}</h3><p>{booked ? "The confirmation reflects every selection made in this demonstration." : "Changing the staff member or date updates the available times. Unavailable slots cannot be selected."}</p>{booked && <button className="demo-reset" onClick={() => setBooked(false)}>Restart booking demo</button>}</div>
        ...
        {demo === "pos" && <div className="pos-demo"><div className="pos-stats"><article><span>Daily sales total</span><b>{orderStep >= 5 ? "₱28,450" : "₱26,950"}</b></article><article><span>Harbor Roast stock</span><b>{orderStep >= 3 ? "3 units" : "5 units"}</b></article><article className="alert"><span>Low-stock warning</span><b>{orderStep >= 4 ? "Triggered" : "Monitoring"}</b></article></div>
          <div className="pos-grid"><div className="order-card"><span>FICTIONAL SAMPLE ORDER</span><h3>Order #DEMO-1048</h3><div><b>2 × Harbor Roast Coffee</b><span>₱1,500</span></div><small>Sample customer: Ana Reyes · Website order</small><button className="demo-primary" onClick={() => setOrderStep(step => Math.min(step + 1,6))}>{orderStep < 0 ? "Trigger sample website order" : orderStep < 6 ? "Continue workflow" : "Workflow complete"} <ShoppingCart size={17}/></button>{orderStep >= 0 && <button className="demo-reset" onClick={() => setOrderStep(-1)}>Reset POS demonstration</button>}</div>
