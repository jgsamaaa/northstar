...
export function ContentPage({ path }: { path: string }) {
  ...
  if (path === "about") return <Shell><PageHero eyebrow="ABOUT NORTHSTAR" title="Global technology experience, focused on Philippine businesses." copy="Northstar combines disciplined execution, practical web development, and a personal commitment to helping businesses build a stronger presence online."/><section className="about-story section-pad"><div><Eyebrow>ABOUT NORTHSTAR SYSTEMS</Eyebrow><h2>Practical digital systems built to help Philippine businesses move forward.</h2></div><div><p>Northstar Systems helps Philippine businesses create a stronger online presence and connect the tools behind inquiries, bookings, sales, inventory, customer support, and follow-up.</p><p>The goal is not to add more software for its own sake. It is to build one clear, practical system that customers can trust and teams can confidently operate.</p></div></section><section className="founder-section about-founder"><FounderPortrait/><div className="founder-copy"><Eyebrow>{siteConfig.founderName.toUpperCase()} · FOUNDER</Eyebrow><blockquote>“{founderQuote}”</blockquote>{founderBio.map(paragraph => <p key={paragraph}>{paragraph}</p>)}<div className="founder-credentials">{founderCredentials.map(item => <span key={item}>{item}</span>)}</div></div></section><FinalCTA/></Shell>;
