...
export function ContentPage({ path, nonce }: { path: string; nonce?: string }) {
  ...
  if (path === "projects") return <SiteShell><PageHero eyebrow="SELECTED PROJECTS" title="Digital work designed around the real job." copy="A growing record of Northstar website concepts and product systems, with each project clearly labeled by its current stage."/><section className="projects-section projects-page"><Reveal className="section-title split-title"><div><Eyebrow>PROJECT INDEX</Eyebrow><h2>Different industries. The same standard of clarity.</h2></div><p>This collection will grow as new Northstar projects are approved for publication. No invented outcomes or client claims.</p></Reveal><p className="portfolio-context"><strong>Portfolio context:</strong> Service tags describe the disciplines demonstrated in each project. They do not, by themselves, claim a paid client engagement, endorsement, or production adoption.</p><ProjectCards/></section><FinalCTA/></SiteShell>;
  if (path === "industries") return <SiteShell><PageHero eyebrow="INDUSTRY SOLUTIONS" title="Built for how your business operates." copy="Northstar adapts to your staff, resources, customers, and operating rules."/><section className="industry-cards section-pad">{industries.map((item, index) => <article key={item.name}><span>0{index + 1}</span><h2>{item.name}</h2><p>{item.problem}</p><div className="recommended-system"><small>RECOMMENDED SYSTEM</small><b>{item.system}</b></div><ul>{item.features.map(feature => <li key={feature}><Check/>{feature}</li>)}</ul></article>)}</section><FinalCTA/></SiteShell>;
  ...
  if (path === "packages") return <SiteShell><PageHero eyebrow="SOLUTION PACKAGES" title="Start with a clear product and a clear price." copy="Choose a focused website package or begin with a scoped booking, POS, inventory, or custom system. Client-owned services and additional work are always identified separately."/><section className="packages-section standalone-packages"><PackagesContent/></section><FinalCTA/></SiteShell>;
